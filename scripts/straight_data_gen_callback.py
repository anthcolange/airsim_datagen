...
class straight_crash:
	def __init__(self):	
		...
		self.cam_im_list = []
		self.state_list = []
		self.callback_thread = threading.Thread(target=self.repeat_timer_callback, args=(self.callback, self.time_step))
...
if __name__ == '__main__':
	x = straight_crash()
	x.start_callback_thread()
	while x.flight_num < 10:
		_ = x.reset_pose()
	x.stop_callback_thread()
